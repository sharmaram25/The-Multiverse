...
import { OrbitControls, Stars, Text, Float, MeshDistortMaterial, MeshTransmissionMaterial, Edges } from '@react-three/drei';
...
function MinimalUniverse({ position, onClick, onHover }: any) {
  ...
  return (
    <Float speed={2} rotationIntensity={0.5} floatIntensity={2} position={position}>
      <mesh
        ref={meshRef}
        onClick={onClick}
        onPointerOver={(e) => { e.stopPropagation(); setHovered(true); onHover(true, 'Minimal'); document.body.style.cursor = 'pointer'; }}
        onPointerOut={(e) => { e.stopPropagation(); setHovered(false); onHover(false, ''); document.body.style.cursor = 'auto'; }}
      >
...
function GlassUniverse({ position, onClick, onHover }: any) {
  const meshRef = useRef<THREE.Mesh>(null);
  const [hovered, setHovered] = useState(false);
  ...
  return (
    <Float speed={2.5} rotationIntensity={1} floatIntensity={2} position={position}>
      <mesh
        ref={meshRef}
        onClick={onClick}
        onPointerOver={(e) => { e.stopPropagation(); setHovered(true); onHover(true, 'Glass'); document.body.style.cursor = 'pointer'; }}
        onPointerOut={(e) => { e.stopPropagation(); setHovered(false); onHover(false, ''); document.body.style.cursor = 'auto'; }}
      >
...
function NeonUniverse({ position, onClick, onHover }: any) {
  const meshRef = useRef<THREE.Mesh>(null);
  const [hovered, setHovered] = useState(false);
  ...
  return (
    <Float speed={3} rotationIntensity={2} floatIntensity={2} position={position}>
      <mesh
        ref={meshRef}
        onClick={onClick}
        onPointerOver={(e) => { e.stopPropagation(); setHovered(true); onHover(true, 'Neon'); document.body.style.cursor = 'pointer'; }}
        onPointerOut={(e) => { e.stopPropagation(); setHovered(false); onHover(false, ''); document.body.style.cursor = 'auto'; }}
      >
...
function BrutalistUniverse({ position, onClick, onHover }: any) {
  const meshRef = useRef<THREE.Mesh>(null);
  const [hovered, setHovered] = useState(false);
  ...
  return (
    <Float speed={1.5} rotationIntensity={0.2} floatIntensity={1} position={position}>
      <mesh
        ref={meshRef}
        onClick={onClick}
        onPointerOver={(e) => { e.stopPropagation(); setHovered(true); onHover(true, 'Brutalist'); document.body.style.cursor = 'pointer'; }}
        onPointerOut={(e) => { e.stopPropagation(); setHovered(false); onHover(false, ''); document.body.style.cursor = 'auto'; }}
      >
...
export default function MultiversePortal() {
  ...
  const handleEnter = (name: string, position: [number, number, number]) => {
    ...
    // Trigger transition after camera animation
    setTimeout(() => {
